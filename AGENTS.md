...
- 不引入 pi、Claude Code 或 deepseek-harness 的任何代码。参照的是设计思想，实现必须独立；每个与它们不同的取舍记入 docs/dev/decisions.md。
- 三个参照源的证据等级不同，引用时必须标明是哪一档（D#69）：pi 是可读源码、
  CC 是反编译源码（行号会漂，检索符号名）、dsh 是第一方源码 + 第一方架构文档
  （唯一一家「设计意图」有官方原文的）。「文档说」与「源码是」在 pai 里从来不是同一句话——
  feature 09、11、13 都撞过；dsh 多了一档官方文档，不代表它免于反向对照。
...
- docs/dev/需求池.md ——「用户想要什么」。用户提的想法先落这里（记原话不转述——转述会丢掉决定性细节），再决定出路：升格立档案 / 降格进 TODO / 划掉不做（保留理由）。与 TODO 的分工是硬的：那边是已确认要做的，这边是还没评估的。
...
档案按「一次交付」切不按模块切——改已交付功能就新建档案并链回旧的，旧档案冻结。
...
- 学习笔记（官方文档精读、源码走读、概念整理、方法论回流、开发中撞出的可迁移工程知识）进 knowledge/，须带统一头部（含 pai 锚点），并登记进 knowledge/README.md 的登记表。目录按来源分（官方文档 / 别人的源码 / 无单一外部原文），选哪个见 knowledge/README.md「结构」。开发知识分两种：只关于 pai 的（为什么这么设计、踩了什么坑）进 docs/dev/，换个项目仍成立的（POSIX 语义、Unicode 宽度这类）才进 knowledge/concepts/。写不出 pai 锚点的内容不进本仓库（去面试准备仓库）——唯一豁免是 knowledge/inbox.md：待消化的新工具/想法一行一项收留，升格成笔记时才须锚点。
...
- 不用加粗（`**…**`）。强调靠句子结构与措辞，不靠字重：一段里到处是粗体等于没有重点，且粗体在纯文本 diff、终端、grep 结果里全是噪音。要突出的东西改成独立一句、独立一条列表项，或写进小标题。
- 不用引用块（行首 `>`）。原来用引用块承载的三类内容改写成正文：引用原文用 `*"…"*` 或反引号包住并在正文里说明出处；旁注 / 提醒（⚠️、🔴、「pai 视角」）直接作为独立段落，开头一句点明它是旁注；代码用围栏代码块，不要套在引用块里。
- 上面两条对 knowledge/ 与 docs/ 下的 md、以及 AGENTS.md 自身一律生效，新写与改写都按这个来。行内代码、围栏代码块内部的原文照抄，不受影响（glob 的 `/src/**`、JSDoc 的 `/** … */` 不是加粗）。
...
- 类型注解必写；不用 Any 除非确有必要。注意：目标运行期是 Python 3.9.6（`pyproject` 写 >=3.9；本项目环境用 mkvirtualenv 的 `~/.virtualenvs/pai`，不激活时系统 `python3` 也是 3.9.6）。`./test.sh` 用当前激活的 python3，pytest 头一行会打印实际版本，跑之前扫一眼。`dict[str, X]`、`list[int]` 这类内置泛型在 3.9 运行期就合法（PEP 585），可直接用；真正需要 `from __future__ import annotations` 的是 `int | None` 这类联合类型语法（PEP 604，3.10 才进运行期）。
- 中文注释只写"代码本身说不出的约束"，不写"下一行在干什么"。
- 类型词汇表（提交与分支共用一套，不另立第二套）：

  | 类型 | 用于 | 判据 |
  |---|---|---|
  | `feat` | 新需求、新能力 | 用户看得见新东西 |
  | `fix` | 修 bug | 修的是已存在的错误行为 |
  | `perf` | 更快 / 更省（token、内存、请求数） | 行为不变、指标变好，且拿得出数字 |
  | `refactor` | 改内部结构，行为与指标都不变 | 外部观察不到任何差异 |
  | `docs` | 文档与知识沉淀 | 不动 src |
  | `test` | 只补测试 | 不动被测代码 |
  | `chore` | 杂务（依赖、配置、构建） | 以上都不是 |

  「优化」不是类型——它是个模糊词，先问优化的是什么，再对到上表：
  体验变好是 `feat`，跑得更快是 `perf`，只是代码更顺眼是 `refactor`。
  说不清属于哪个，通常说明这次改动还没想清楚要什么。
  `perf` 尤其：没有前后数字就不是 perf（同「数字要真实」那条规矩）。

- 提交信息格式：`<类型>(module): message`。永远不要未经要求就 commit。
- 分支命名：`<类型>/<NN>-<小写连字符描述>`，且新需求一律开新分支，不在 main 上做。
  类型取上表（形状参照 [Conventional Branch](https://conventionalbranch.org/) 的
  `<type>/<description>`，但词汇用本仓库这一套，避免同一件事两套说法）。
  例：`feat/08-storage-layout`、`fix/12-repl-crash-on-eof`、`perf/summary-prompt-cache`。
