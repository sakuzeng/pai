...
- **三个参照源的证据等级不同，引用时必须标明是哪一档**（D#69）：pi 是**可读源码**、
  CC 是**反编译源码**（行号会漂，检索符号名）、dsh 是**第一方源码 + 第一方架构文档**
...
- **类型词汇表（提交与分支共用一套，不另立第二套）**：
  ...
  | 类型 | 用于 | 判据 |
  |---|---|---|
  | `feat` | 新需求、新能力 | 用户**看得见新东西** |
  | `fix` | 修 bug | 修的是**已存在的错误行为** |
  | `perf` | 更快 / 更省（token、内存、请求数） | 行为不变、**指标变好，且拿得出数字** |
  | `refactor` | 改内部结构，行为与指标都不变 | 外部**观察不到任何差异** |
  | `docs` | 文档与知识沉淀 | 不动 src |
  | `test` | 只补测试 | 不动被测代码 |
