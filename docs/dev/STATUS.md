...
| `core/permissions.py` | 可用 | 规则解析 + 七步求值链（deny → 危险路径 → 显式 ask → bypass → acceptEdits → allow → 兜底；顺序不许改 D#46）；兜底是工作目录边界函数不是常量（D#51）；权限模式四态（D#53）；两层 `settings.json` |
...
- `./test.sh` → 1160 passed, 3 deselected，全部离线，约 112s。这是默认路径。
