...
`/mode` 与 shift+tab 要能切，gate 就必须读一个**可变持有者**而不是捕获的常量。
...
- **`commit(lines)`：把内容从 dock 上交到 scrollback**——先清掉 dock，把 lines
  ...
  测试要钉死：commit 之后**再重画 dock 不会覆盖刚上交的内容**。
...
- **bracketed paste**：`\x1b[200~…\x1b[201~` 之间的内容整体插入，其中的回车**不提交**。
...
- `render(width)` 在光标处吐 `CURSOR_MARKER`，且**该标记不占可见宽度**。
- 历史 ↑/↓：读 05 已交付的历史文件，语义不变（按 cwd 分文件、连续重复只记一条）。
- 续行：行尾 `\` + 回车不提交，进入续行态。

**绿**：`keys.py` + `editor.py`。编辑器是**纯状态机**：`(state, key) -> state`，不做 IO。
...
- 仲裁函数：给定（输入框内容、最后按键时刻、待决对话框队列、用户是否主动唤出），
  返回**谁拥有输入**。
- 输入框非空 → 返回「编辑器」，待决对话框**不弹**。
- 输入框非空但距最后按键 ≥1500ms → 返回队首对话框。
- 输入框清空 → **立即**放行（不等 1500ms）。
- 用户主动唤出的对话框不受抑制。
...
**红**（≥7 条）
- `make_before_tool_call` 接受**可变模式持有者**；改持有者后，**下一次判定就用新模式**
...
- 状态行显示当前权限模式 + 待决请求数。
- **拿真实会话轨迹当输入**跑一遍事件序列（AGENTS.md：编的字符串测不出中文与
...
**红**（≥6 条）
- **非 tty 闸门**：`stdout` 不是 tty 就**整个不进 TUI**，退回今天的行为（判 stdout 不判 stdin，与 CC 同口径）；顺带处理管道下的欢迎语与提示符噪音。
...
- **主线程断言**：不在主线程时**明确告警**，而不是像今天这样静默退化为不可中断
...
- `./test.sh` 全绿，新增测试**不少于 40 条**（本 plan 各 task 下限相加 = 67，
