...
| 功能 | 状态 | 一句话结果 |
|---|---|---|
| [40-20260826-old-debts](40-20260826-old-debts/README.md) | 已交付 | 还三类旧账（用户点名），共同点是判据早就写下来而没人回来查。①共享测试夹具层：跨测试文件的 import 归零（此前 5 处、两种写法），`tests/helpers.py`（动作）与 `tests/trajectories.py`（数据）分家，`_OPEN` 5 份→1 份、reader 5 个变体→1 个；②拆文件一个不拆一个真拆——`compaction.py` 重估后不拆（触发条件字面满足，但条目自带的参照 pi 893 行与 AGENTS「一个阶段一个模块」都不支持），`interactive.py` 1365 → 1007 抽出 `modes/commands.py`（判据是「谁在共用」不是「哪块最大」），中断三件套搬回 `core/interrupt.py`；③方法论欠账清 6 条（green-but-which-path、todo-drift 两篇新的 + 注入反证第五坑、raw mode 接管清单两处追记）。全程不改行为：1503 passed 一个数字没动 |
| [39-20260826-busy-heartbeat](39-20260826-busy-heartbeat/README.md) | 已交付 | 干活期间的键盘不再像死了（用户从四处体验候选里点名；12 复盘质疑二，登记后挂了十五天）：`core/heartbeat.py` 进程级心跳 + `shell._wait` 每轮一跳 + TUI 的 `pump_keys()` 由事件路径与心跳共用。顺带照出并修掉一个谁也没登记过的真 bug：TUI 里 `!命令` 期间 Ctrl+C 停不下来（raw mode 关了 ISIG，而那时没人读键盘）。两条 e2e 先红后绿；三处注入反证里有两处没红，照出的都是我自己写坏的地方（多余的 refresh、心跳装卸不对称）。1500 passed |
| [38-20260826-offline-real-gap](38-20260826-offline-real-gap/README.md) | 已交付 | 缩小「离线全绿」与「真实使用」的差距（用户指示）。动手前的发现就是引子：假 provider 对每轮回固定 usage，而锚点簿记的正是这个数、切点靠相邻锚的差值反推——差值恒为 0 意味着自动压缩在所有 e2e 里结构上走不到，而没有任何测试会因此变红（2026-08-10 用户真跑报告过「压缩一次都没触发」，两边十几天没接上头）。三类差距各自兑现：①假 provider 的 usage 改成随对话增长 → 自动压缩第一次在真进程 + 真 pty 里跑到（屏幕上有「压缩：切于 2」、摘要请求真的发出去了，顺带把「两步节奏」从设计推论变成事实）+ 流中途 Ctrl+C e2e；②remember→召回全链、规则模型听不听写成 `--llm` 冒烟（默认跳过）；③IME/resize/颜色/鼠标做成手工清单（六节，如实写明尚未被人跑过）。外加仓库自己成为 feature 36 的第一个用户（`.pai/rules/tests.md`）。知识回流：instruments-lie 从四种骗法扩到五种。1492 passed |
| [37-20260826-context-rewritten-event](37-20260826-context-rewritten-event/README.md) | 已交付 | `on_context_rewritten` 升格成事件（refactor，用户指示；行为逐字不变）：`events.CONTEXT_REWRITING = (Compacted, ConversationCleared)` 成为「哪些事件意味着上下文被改写」唯一的家，装配层给出事件监听器并联进 on_event（三处：once、REPL、TUI 自建 handler——与既有的 trace 同一处安排），`run_agent` 的参数与八个调用点的穿参数全部消失（28 处引用 → 0）。形状三选一拍板 A：不给同一件事造第二个名字（候选 B 的新事件会让同一位置发两条）。纵切（两轮 REPL 中间 /clear，召回必须能重来）+ 三处注入反证。自己造了一个假测试又自己照出来（TUI 那条钉的是引爆点之后才跑到的行），记进复盘。1487 passed |
| [36-20260826-path-scoped-instructions](36-20260826-path-scoped-instructions/README.md) | 已交付 | 路径作用域规则（需求出处：用户 2026-08-19 在需求池点名，2026-08-26 在「下一程接哪件」里选中）：pai 的指令层从「只有常驻一档」变成两档——`.pai/rules/*.md` 与 `~/.pai/rules/*.md` 里带 `paths:` 的规则，只在模型这一步真的碰到匹配文件时才进上下文。挂点在 loop 的工具结果回填处（路径怎么取仍下放给 `Tool.get_path`），注入形态与召回块同款，压缩后靠 `on_context_rewritten` 作废重来（feature 35 那条通道的第二个消费者）。三问一轮拍板全 A，另立 D#75（不带 `paths:` 的文件不加载——与官方相反，降低常驻成本的功能里不该再开一条常驻通道）。自写 glob→正则（`fnmatch` 的 `*` 跨 `/`，语义是错的）。交付中自查出一个真 bug：相对路径按项目根解析而模型给的是相对 cwd 的（子目录启动即错）。+33 测试，1481 passed |
| [35-20260826-todo-backlog-batch-2](35-20260826-todo-backlog-batch-2/README.md) | 已交付 | TODO 存量批清第二轮（同一套挑选标准，另加两条验收要求：缺测试的要做注入反证、只改表示层的验收是「值逐字相等」）：12 条真修 + 3 条对账核销，开放项 165 → 156（含 5 条本轮新登记）。这次动手前先拿代码复核候选，15 条里 5 条与条目描述对不上（3 条漏勾、1 条修法照做会引入新 bug、1 条只写了一半）。真修面——`PAI_KEEP_RECENT_TOKENS`（压缩链路第一次能在真实使用里跑到）、`/compact` 说清还差多少 token、上下文被改写就清召回去重表（10 遗留 6 + 揪出 `/clear` 同族洞，用户拍板「全清」）、召回单篇 4000 字符上限、`/memory reload`、`AGENTS.md` 进指令发现（D#43 复议，用户拍板）、子目录懒加载注释改说真话、能力判定器崩溃留痕、`derive_replay` 不再把指令消息当任务、`--resume` 说清设置不跟着回来、`./test.sh --fast` 快循环（4.5×）、宽度原语搬进 `tui/width.py`（36810 次值比对全等）。1447 passed |
| [34-20260825-todo-backlog-batch](34-20260825-todo-backlog-batch/README.md) | 已交付 | TODO 存量批清（用户给的挑选标准：只做「不等外部输入、修法已有形状」的，等数据/等复现/记录性的一律不碰）：11 条真修 + 6 条对账核销 + 2 对重复登记合并，开放项 180 → 161。真修面——`PAI_CONTEXT_WINDOW` 非法值清晰报错、锚点换具名 `Anchor(index, tokens)`（顺带钉住「无锚判据是 index is None」）、未知 role 照常称重（推翻 D#8 的秤那一半）、熔断单向性补测试（注入反证）、TUI 下记忆/召回事件走 `EventSink` 不再打进 stdout、队列 `__len__`、`--resume` 同秒 tie 排序稳定、`read_file` 截断提示给出路、`core/protocols.py` 的 `ChatClient`（R#14）、design_gate 类型注解、R3 风格五条。两问拍板全 A。1411 passed |
