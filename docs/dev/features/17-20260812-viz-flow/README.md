...
改已交付功能 [01-viz](../01-20260803-viz/README.md) 的结果（规矩 7：改变而非完成），
...
1. 终端真跑一个回合，浏览器不点刷新即可看到：结构图节点按序点亮 + 新 turn 卡片出现；
...
4. 每个 pipeline 节点、每张工具卡、每类 harness 事件标注代码位置；
...
- 候选 A·工具自动自省 + 其余手写映射（推荐）：工具卡的 `file:line` 从注册表函数对象
  ...
  pipeline 节点与 harness 事件类型 → 代码位置是手写映射（`PermissionDecided → core/gate.py`
...
- 候选 A·HTTP 轮询 + 行号游标（推荐）：`/api/events?cursor=N`，前端 2s 一拉，
  ...
  注：这会推翻 01 的「不做自动刷新（YAGNI）」——当时不做的是静态结构图的刷新，
...
「计费」。用户指出缓存命中便宜 50 倍，这么加既不是钱也不是上下文大小。改为三个
