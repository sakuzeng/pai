...
- 取**git 仓库根**（保持官方语义：同仓库的子目录与 worktree 共享一份数据），
  不在 git 仓库里就取当前目录本身；
...
- memory 与 sessions 落在**同一个** `<project_dir>` 下。
- 同一 git 仓库的两个子目录 → 同一个 `project_dir`。
