...
2. `test_slug_uses_the_git_root` —— 临时 `git init` 的仓库，两个子目录得到**同一个** slug。
...
5. `test_known_slug_collision_is_documented` —— **把已知缺陷钉成测试**：
   断言 `/a-b/c` 与 `/a/b-c` **确实**撞成同一个 slug，docstring 写明这是与 CC 一致的
...
2. `test_slash_memory_shows_both_memory_and_session_dirs` —— `/memory` 要把**会话目录**
