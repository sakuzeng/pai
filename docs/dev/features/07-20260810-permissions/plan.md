...
3. `test_permission_layer_never_branches_on_tool_name` —— 白盒：给一个假工具挂
...
5. `test_env_runners_are_not_stripped_and_this_is_a_known_hole` ——
   `allow=["Bash(devbox run *)"]` 确实放行 `devbox run rm -rf .`。
...
3. `test_single_slash_anchors_to_the_settings_source` —— 用户设置里的 `/secrets/**`
   指向 `~/.pai/secrets/**` 而不是项目里的 `secrets/`（官方最大的坑）。
...
8. 注入验证（roadmap 硬要求）：把 `decide()` 的求值顺序改成「allow 优先」，
   断言 Task 1 与 Task 3 的关键测试确实变红；还原后再绿。整段过程贴进 devlog。
