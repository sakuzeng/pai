...
feature 06 交付的自动记忆只有一层：`MEMORY.md` 索引常驻上下文，主题文件「靠模型自己
...
- 按 mtime 新→旧排序，截断 200 个（`MAX_SCANNED`）；
...
此时上下文里那份是对的（每次都重新扫），错的只是给人看的那份，下次写记忆自动修好。

相对时间只进上下文，不进盘：`render_index(headers, now=None)` —— `now` 为 None 就
不渲染时间。理由：相对时间是渲染时刻的函数，写进持久文件就会腐坏，
...
- `freshness_note(days)`：≤1 天返回空（新鲜时警告是噪音，CC 同款阈值）。
  ≥2 天返回：「这条记忆写于 N 天前。记忆是时间点观察，不是实时状态——
...
2. `already_surfaced` 在调模型之前滤掉（否则 5 个名额浪费在已经在上下文里的东西上）；
...
- 注入位置：该轮 task 消息之后追加一条 user 消息。append-only，与 `SessionLog`
...
3. 删掉一个记忆文件后再 `remember`，`MEMORY.md` 里那行消失（投影方案的判据测试——
   账本实现过不了这条）；
...
8. 至少一条测试拿真实会话轨迹当输入（AGENTS.md 测试规约）。
...
走读笔记未覆盖、由这份样本补上的四条：`description` 与索引钩子是两个不同字符串；
