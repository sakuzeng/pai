...
`./test.sh` 全绿（基线 458）、装配器对真实 chunk 夹具跑通、usage 两种协议形状都取得到、
...
`set_origin_session`）「一旦有并发就要重新考虑」——查过了，它们是装配期写、执行期只读，
...
且必须先改 `Tool.run` 返回契约（不知道工具是否出错就没法决定杀不杀兄弟），
...
我给出的评估（用户采纳）：steering 需要的是独立输入线程或非阻塞 stdin，
...
| 权限按批前置 + 结果按原顺序回填 + SessionLog 加锁 | `core/loop.py`、`core/session.py` |
...
8. 核实并核销一条旧担忧：TODO 里「三个进程级全局一旦有并发就要重新考虑」——
   查证不成立（`set_memory_dir` / `set_notifier` / `set_origin_session` 都是
...
- [K streaming/streaming-tool-calls.md](../../../../knowledge/streaming/streaming-tool-calls.md)
  ...
  usage 位置、中断无 usage、并行调用不会重复计 usage。
...
落笔时发现官方 Claude Code 文档没有 streaming 章节
...
1. `stream_options.include_usage` 在 DeepSeek 上是空操作，且没有文档所说的
   「choices 为空数组的额外块」——usage 一律在带 `finish_reason` 的末块上。
...
3. 中断掉的流拿不到 usage（末块没读到），被中断请求的消耗恒定少算。
   原本不在任何清单上。
