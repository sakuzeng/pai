...
- 新增 `knowledge/`（选址取舍见 D#35）：README（索引 + 准入规约 + 模板）+ 4 篇种子笔记——
  ...
  关键结论现场核对过 guards/ 原文）、`source-walks/cc-compaction.md` 与
  `source-walks/pi-agentloop.md`（指针笔记，深度内容链接面试准备仓库不搬运）、
  `claude-docs/context-management.md`（官方 context-window/costs 页精读，
...
- `knowledge/claude-docs/map.md`：官方章节 → 阶段归属/明确不做 的覆盖表。
