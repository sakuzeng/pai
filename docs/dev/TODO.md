...
- [ ] ★ 拖选卡顿的真实成因**至今未确诊**，feature 16 的节流没解决它
...
- [ ] `memory.py:57` 的注释「官方同款语义，模型要用时自己 read_file」不成立（准确度，先改）：
      官方对 cwd **之下**子目录的 `CLAUDE.md` 是**框架懒加载**——发现了但不在启动时加载，
...
- [ ] 没有路径作用域规则（对应官方的 `.claude/rules/` + `paths:` frontmatter）—— 同上，已转需求池：
      ...
      Claude 读到匹配文件时才加载。这是**降低常驻成本**的机制，而常驻那一层
...
- [ ] `remember` 在真实会话里从未被调用过（2026-08-19 实测）：
      ...
      那条验的是侧查询本身（已完成 2026-08-11），这条验的是**从写入到召回的整条链**。
