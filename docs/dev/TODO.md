...
### feature 44（markdown 渲染）遗留与发现 —— 2026-08-26

档案：[features/44](features/44-20260826-markdown-rendering/README.md)

- [ ] 英文按字符折行，不按词折行（44 复盘质疑一，比一般遗留严重）：
      `_wrap_spans` 逐字符折，于是英文单词会被从中间劈开。我给 `NO_LINE_START`
      塞了 ASCII 的 `,.;:?!)]}%` 去补最刺眼的症状，但那是**用一条排版规则去补
      我自己的折行算法造成的问题**——英文里标点顶行首本来就不该发生。
      正解是英文按词折行（空格处优先断）。这不是「还没做的功能」，是做错了一半又绕开了。
- [ ] 「模型写下换行就是想换行」是个未验证的假设（44 复盘质疑三）：
      本轮据此把单换行做成硬换行。但模型也可能只是把长句在源码里折了行
      （不少模型输出会在 80 列附近断），那样硬换行会把一句话切成两行显示。
      没有数据，只有一次真跑的印象。验法：攒几十条真实回答看换行分布。
- [ ] REPL / once 仍是原文（44 遗留）：拍板问 1 的直接代价，不是疏漏。
      要做的话得先解决「逐字上屏 vs 攒完再渲染」的冲突，那是独立需求。
      与 43 遗留的「`render_text` 返回一行的契约」是同一条线上的两件事。
- [ ] 框线字符的 Ambiguous 宽度（44 遗留）：`─ ━ │ • ◦ ┌ └` 在部分 CJK 终端
      按 2 列画，会让标题下划线、列表、代码块左栏整体右移一格。
      表格分隔行已避开（用 ASCII `-`），其余没避。要彻底解决得让宽度表可配，
      那是 `width.py` 的事。
- [ ] 表格极窄时每列只剩一个省略号（44 遗留）：`MIN_COL_WIDTH = 3` 在
      「宽度 / 列数」小于 3 时压不住，退化成 `…  …  …`。
      拍板问 4 明确否掉了键值块降级，所以这一格暂时就是这样。
- [ ] 代码块没有语法高亮（44 遗留）：整块一个颜色。做的话要么引 pygments
      （一个新依赖），要么自己写关键字表（每加一门语言一张表）。
- [ ] 禁则只做了避头没做避尾（44 遗留）：行尾出现开引号/开括号同样不美。
      没做是因为要往回看更多字符，且实测里模型的回答几乎撞不到。
- [ ] 排版类断言要对宽度区间跑，不是对单个宽度跑（44 复盘「下次怎么做更好」）：
      本轮的禁则测试用 `for width in range(8, 40)`，一写出来就抓到了边界上的
      字符丢失；而所有单点宽度的断言都放过了它。
      落点：表格、折行、缩进这些「特定尺寸下才显形」的判据都该带一个循环。

### feature 45（真用一次）发现 —— 2026-08-27

档案：[features/45](features/45-20260827-first-real-use/README.md)、
[发现清单](features/45-20260827-first-real-use/evidence/发现清单.md)

- [x] ~~给 `search_files` / `run_tests` / `git_read` 补 system prompt 引导（45-A1，
      **本清单里收益最大的一条，且已有 A/B 证据**）：`build_system_prompt` 里
      今天只有一句工具引导，是给 `edit_file` 的——而模型确实听了那一句。
      实验组加三句后：`run_tests` 从「任务明说跑测试却零次被选中」变成
      1 次调用、0 弹窗、9 秒完成。三行改动。
      诚实边界（45 复盘质疑二）：整体 bash 调用只从 4 降到 3，
      引导对「有专用工具且意图明确」的场景有效，对探路型步骤基本没用。~~
      已做 2026-08-27（feature 46）：五句条件化引导（含 read_file / list_dir）+ bash 描述收紧；
      验收是 `tests/test_llm_steering.py` 三条 `--llm` 测试，真跑三遍全过。
- [x] ~~没有工具能回答「这个项目长什么样」（45-A2）：`pwd && ls` 是每个会话的
      必然开场，而它必然弹窗。`search_files(pattern="")` 只给扁平文件名列表，
      给不出目录结构。这是 41/42「把活从 bash 摘出来」漏掉的一格。~~
      已做 2026-08-27（feature 46）：装配期注入 cwd + 结构摘要（顺带修掉 45-C4），
      加 `list_dir` 工具兜底会话中途新建的目录。
- [x] ~~`search_files` 不接受单个文件当搜索根（45-A2，当场验过）：
      `search_files(pattern="x", path="a/b.py")` → 「搜索根不是目录」。
      要在一个已知文件里找一行，今天只能走 bash。修法很小：`path` 是文件时
      直接搜它。~~
      已做 2026-08-27（feature 46）：`path` 是文件时就搜它；吃目录的行为逐字不变（有回归守卫）。
- [ ] markdown 折行把英文单词从中间劈开（45-B1）：真跑每屏都有
      （`rul`/`es.py`、`MAX_OUTPUT_CHA`/`RS`、`read_fi`/`le`）。
      **这条已被推迟一轮**——44 复盘里我自己承认「用 ASCII 标点避头去补自己
      折行算法造成的问题」。真跑把它从「欠账」升级成实测确认的高频可见缺陷。
- [ ] 编辑的 diff 在默认视图里看不见（45-B2）：feature 43 让 `edit_file` /
      `write_file` 回一段 diff，但 TUI 工具结果行默认折叠，要按 `^O` 才展开。
      「改了什么看得见」这个需求在默认视图下没有兑现。
      修法方向：编辑类工具的结果默认展开（它们行数少且是用户最关心的）。
- [ ] 权限弹窗文案是内部术语（45-C1）：「`bash` 不参与工作目录边界判定
      （未声明路径语义），按最保守处理」——对用户说 D#52 的实现细节，
      既没说清风险，也没给出路。诚实边界（45 复盘质疑一）：我自动应答了每一个
      弹窗、根本没读文案，所以这条的严重程度我判断不了，要等真人跑。
- [ ] 计时器把等人回答的时间算进「用时」（45-C2）：第一次真跑显示 `⠹ 3m59s`，
      其中 3m55s 是弹窗在等我。等人时该停表。
- [ ] 默认 token 预算撑不过一个中等任务（45-C3）：`--max-tokens` 默认 200k，
      实测一个 11 步的真实任务（改函数 + 加测试 + 跑测试）烧到 131820 就撞顶。
      与「预算改按钱算」那条相关但不同：那条讲计价，这条讲默认值定得对不对。
- [ ] 模型不知道自己的 cwd（45-C4）：系统提示里没有工作目录，于是它每条 bash
      都写 `cd /abs/path && …` 或 `git -C /abs/path …`，浪费 token 且让权限
      弹窗里的命令长得没法读。修法：system prompt 里带一行 cwd。
- [ ] 长 cwd 把 footer 撑满（45-B3）：模型名与上下文占比被整条挤掉。
      footer 该按信息优先级取舍（路径可以中间省略），不是先到先得。
- [ ] 手工清单仍未被真人跑过（45 遗留，接 38 遗留）：收紧版在
      `features/45/evidence/手工清单-收紧版.md`，六节，每条给出「什么算过」。
      我没有 IME、没有真鼠标、改不了终端主题，这半只能等用户。
- [ ] 加工具的拍板里固定加一问「模型怎么知道该用它」（45 复盘「下次怎么做更好」）：
      交付验收要有一条「真模型会选它」，哪怕只是一次十几秒的真跑。
      这一条比所有单测都更接近「这个功能有用吗」。

### feature 46（工具引导与项目结构）遗留与发现 —— 2026-08-27

档案：[features/46](features/46-20260827-tool-steering/README.md)

- [x] ~~端到端效果没有可信的测量方法（46 遗留，比一般遗留严重）：单次真跑方差极大
      ——同一个问题两次跑出 7 调用/2 弹窗与 12 调用/5 弹窗。`--llm` 测试只验
      「给定意图会不会选对工具」，验不了「整条轨迹变好了没有」。
      没有这个，以后所有「让 agent 更聪明」的改动都只能靠感觉。
      方向：固定一组任务跑 N 次取中位数，但那要花钱花时间，值不值得先想清楚。~~
      部分已做 2026-08-27（feature 47）：单步工具选择的可重复测量
      （`./eval.sh --llm -k tool_choice`，18 case × 5 采样 × 2 变体）。
      **只解决了一小块**——它测第一步选哪个工具，测不了整条轨迹，
      而「整条轨迹变好了没有」仍然没有手段。剩下那块另立条目见下。
- [ ] 「加工具的边际收益在递减」这条没被正面对待（46 复盘质疑二）：`list_dir`
      是第七个工具，真跑里被选中过一次、也被 `ls` 顶掉过一次，而它每轮对话都要
      花 schema token。加第八个工具之前该先算这笔账。
- [ ] 结构摘要的三个常数只在本仓库调过（46 复盘质疑三）：`MAX_ENTRIES=120`、
      `MAX_FILES_PER_DIR=8`、`DEFAULT_DEPTH=2`。本仓库 14 个顶层目录 + 深层
      docs/knowledge 树，形状很特别；换个只有 src/tests 的项目深度 2 可能太浅。
      **这是第五个「看着顺眼就定了」的常数**（前四个见 44 那条）。
- [ ] 结构摘要不尊重 `.gitignore`（46 遗留，拍板问 3 的直接代价）：
      本仓库的 `pai_playground/` 是 gitignore 的，照样出现在摘要里、占着预算。
- [ ] 引导句已经五句都堆在 system prompt 里（46 遗留）：加第六个工具时这一段
      会开始显得长，且每轮都要发。触发条件：第六句出现时考虑换形态
      （比如只在相关工具首次可用时说一次）。
- [ ] 档案里「观察」与「测量」要在格式上分开（46 复盘「下次怎么做更好」）：
      「结果与总结」只许引用可重复的测量；单次真跑一律进 devlog 的观察一节，
      且必须连不利的那次一起贴。本轮已按这个做，但它还只是我的自觉。

### feature 47（工具选择评测）遗留与发现 —— 2026-08-27

档案：[features/47](features/47-20260827-tool-choice-eval/README.md)

- [ ] 引导对 bash 场景的回归没修完（47 实测，**这是我 feature 46 引入的**）：
      bash 该赢的 4 条上，有引导 2.60/4 而无引导 3.00/4。
      「起一个 http server」5 次里 5 次去选 `list_dir`。本轮补了一句给 bash
      划回地盘（2.40 → 2.60），没修完。
- [ ] 整条轨迹仍无测量手段（47 遗留，接 46 那条）：单步测量测不出
      「第一步选对、后面仍然绕路」。这是原始问题里没被解决的那一半。
- [ ] 案例集与阈值都没有依据（47 遗留）：18 条是我一个人写的，
      覆盖面无据；`SAMPLES=5` 与两条地板（75%、2.5）都是看着定稿那组数定的。
      **这是第七、八、九个「看着顺眼就定了」的常数**。
- [ ] 用同一组 case 既发现问题又验证修复（47 复盘质疑二）：我补的那句 bash 引导
      是**看着失败的 case 写的**，写完复测好一点就留下了——不折不扣的过拟合。
      正确做法是把案例集切成开发集与保留集，改提示词只看开发集。
      本轮没做的理由是 18 条切一半就更没统计意义，那说明案例集本来就太小。
- [ ] 「整体分不出来」这个说法被无关 case 稀释了（47 复盘质疑一）：
      18 条里真正针对 feature 46 那几句引导的只有几条，
      其余（remember / write_file / chmod）与引导无关，两个变体当然一样。
      更诚实的说法是「相关 case 上有来有回，整体被稀释成零」。
      改法：报告时按「与被改动的引导相关 / 无关」分组给数。
- [ ] 跑一次 4 分半 / 约 360 次请求（47 遗留）：改一句提示词就跑一次的成本不低，
      实际会变成攒着一起跑——而那样「是哪条改动带来的变化」又回到猜的状态。
      可能的方向：只跑与本次改动相关的那几条 case。
- [ ] 案例集改一条前后就不可比，而没有机制提醒（47 遗留）：本轮已经发生过一次
      （v1 → v2）。现在只有 `evals/cases.py` 里一句话。
      方向：把案例集哈希连同结果一起写进 `runs.jsonl` 工件。
- [ ] 新测量的第一条断言应该是「它自己稳不稳」（47 复盘「下次怎么做更好」）：
      本轮的重复性检查是顺手加的，结果是四条里唯一真正救了这一轮的——
      没有它我会拿着 15 vs 14 得出「引导略有效」，而那个差值完全在噪声里。
