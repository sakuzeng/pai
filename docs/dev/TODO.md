...
- [ ] `on_context_rewritten` 该升格成事件的证据更足了（36 复盘质疑二，
      与 35 复盘质疑二同一条）：现在有两个消费者（召回去重表 + 规则注入表），
      而 loop 已经在同一位置发 `Compacted`。拖着的真实成本是每加一个消费者就要
      多穿一遍参数——feature 36 就穿了六个调用点。连带质疑「两个还不构成模式、
      第三个再抽」这套说辞被用得太顺手（34 复盘质疑一的 `Ref[T]` 用的是同一套）。
- [ ] 「两个坐标系恰好相等的测试证明不了任何东西」值得记进方法论
