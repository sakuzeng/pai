...
| 源 | 本机位置 | 证据等级 | 它擅长回答什么 |
|---|---|---|---|
| **pi** | `~/improve/coding/agent/pi-mono`（TypeScript，MIT） | **可读源码**，作者原意只能从代码与注释推 | 「一个最小 harness 的骨架长什么样」——loop、compaction、队列的**结构**最清楚 |
| **CC** | `~/improve/coding/agent/projects/claude-code-source-code`（反编译） | **反编译源码**，⚠️ 行号会漂，检索符号名不记行号 | 「真实用户量验过的默认值是什么」——权限、记忆、TUI 的**取舍**只有它有答案 |
| **dsh** | `~/improve/coding/agent/projects/deepseek-harness`（TypeScript + Python，MIT，pin `47f9438` / 2026-08-13） | **第一方源码 + 第一方架构文档**（`docs/*.zh.md`，唯一一家设计意图有官方原文的） | 「一个团队会把这套东西**说成**什么样」——事件生产/消费、能力 seam、工具执行流水线有成文的名字与边界 |
...
可拿的是**它对问题的切分与命名**，不是它的装配方式；
② 它自称**开发者预览、破坏性变更在即**，所以引用一律带 commit hash，别写「dsh 是这样的」；
③ 它有第一方中文文档这件事**会诱人跳过反向对照**——而 feature 09 的教训正是
