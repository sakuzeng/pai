...
## 「参照」这一栏可以填谁（2026-08-13 起三家，D#69）

| 源 | 本机位置 | 证据等级 | 它擅长回答什么 |
|---|---|---|---|
| pi | `~/improve/coding/agent/pi-mono`（TypeScript，MIT） | 可读源码，作者原意只能从代码与注释推 | 「一个最小 harness 的骨架长什么样」——loop、compaction、队列的结构最清楚 |
| CC | `~/improve/coding/agent/projects/claude-code-source-code`（反编译） | 反编译源码，⚠️ 行号会漂，检索符号名不记行号 | 「真实用户量验过的默认值是什么」——权限、记忆、TUI 的取舍只有它有答案 |
| dsh | `~/improve/coding/agent/projects/deepseek-harness`（TypeScript + Python，MIT，pin `47f9438` / 2026-08-13） | 第一方源码 + 第一方架构文档（`docs/*.zh.md`，唯一一家设计意图有官方原文的） | 「一个团队会把这套东西说成什么样」——事件生产/消费、能力 seam、工具执行流水线有成文的名字与边界 |

三家不是三票，遇到分歧不投票（D#68 那条一般化规矩照旧适用）：
先问「这家的做法里，哪一半是被它自己的其他约束逼出来的」，剥掉之后剩的才可拿。

dsh 的特殊代价，动工前必须记住三条：
① 它是 Cordis 插件架构（一切皆插件），pai 是单体 Python，架构层面不可直接对拿——
可拿的是它对问题的切分与命名，不是它的装配方式；
② 它自称开发者预览、破坏性变更在即，所以引用一律带 commit hash，别写「dsh 是这样的」；
③ 它有第一方中文文档这件事会诱人跳过反向对照——而 feature 09 的教训正是
「精读覆盖了文档每一节，却漏掉一整层机制」。文档更好 ≠ 可以不跑。
...
feature 12 又撞出六条，其中一条改写了一条已登记遗留的定性。
...
- 档案：[features/02-20260803-compaction/](features/02-20260803-compaction/README.md)。
  ⚠️ 诚实边界（2026-08-10 用户实测暴露）：压缩链路在真实使用中一次都没被走到过——
...
- 前置精读（前两条是 REPL 半程的，后两条是 TUI 半程的）：
  ...
  - [x] [knowledge/tui/pi-tui-main-screen.md](../../knowledge/tui/pi-tui-main-screen.md)（绘制侧：Component 四成员契约、差量重绘 diff 的是整份文档的行数组、宽度一变就 `\x1b[3J` 清 scrollback、CURSOR_MARKER 与 IME、超宽行 fail-loud）
  ...
  - [x] [反向对照 → features/12 evidence](features/12-20260811-tui/evidence/20260811-终端反向对照/说明.md)
        （撞出 6 条：pai 对 resize 一个字节都不发；「干活时打的字」其实没丢、
...
- 前置精读（第三批：alt-screen，2026-08-11 补）——为复议原则 2 而读，
  归 [features/13](features/13-20260811-alt-screen/README.md)。前两批那四篇都是给
  ...
  - [x] [knowledge/tui/pi-alt-screen.md](../../knowledge/tui/pi-alt-screen.md)
        （`tui-plan.md` 主体 + `tui-alt-screen.ts`：alt 是另一个渲染器不是补丁；
        follow-end 状态机；退出时要重渲染完整文档打回主屏；
        并纠正一处 pai 自己的转述——原则 2 的原文是「别在 main-screen 里假装」，
        不是「别做 alt-screen」）
  - [x] [knowledge/tui/cc-alt-screen.md](../../knowledge/tui/cc-alt-screen.md)
        ...
        CC 对外部用户默认关这个形态 + 三个逃生口；命中测试 130 行便宜、
  ...
  - [x] [反向对照（动工前）→ features/13 evidence](features/13-20260811-alt-screen/evidence/20260811-alt-screen反向对照/说明.md)
        ...
        （推翻 CC 自己一处注释）、DECRQM 在 Terminal.app 完全不可用且会污染测量、
...
- 档案：[features/07-20260810-permissions/](features/07-20260810-permissions/README.md)
  ...
  hook 崩溃不阻断（D#50）。
  门禁必须带测试这条 anna 回流已兑现：三条注入反证（翻求值顺序 / require_all 恒 False /
  不拆复合命令）各自打红了不同的测试集，且自举跑通了 pai 自己的 `guards/design_gate.py`。
- 补课档案：[features/09-20260810-working-dir-boundary/](features/09-20260810-working-dir-boundary/README.md)
  ...
  阶段 4 交付的是引擎，策略是空的——默认兜底是常量 `allow`，不配置就等于没有权限层。
...
- 前置精读：
  ...
  - ~~官方 streaming 相关章节 → 届时落 knowledge/claude-docs/~~ —— 2026-08-11 改写，理由留档：
    官方 Claude Code 文档没有 streaming 章节（见 [overview/claude-docs-map.md](../../knowledge/overview/claude-docs-map.md) 的覆盖图），
...
- 档案：[features/11-20260811-streaming/](features/11-20260811-streaming/README.md)（6 task TDD，509 passed）。
  交付形态是方案 B：做了流式输出 + 能力标志 + 保序并发；没做边流边派发与兄弟取消
