...
**`apiKeyHelper`**：settings 里配**一条命令**，CC 执行它拿 key。配套机制不小：
...
用途是**密钥轮转与企业网关**——key 不是静态字符串而是「一条能生成 key 的命令」。

**`--bare` 模式**是另一条值得记的设计：hermetic auth，**只认** `ANTHROPIC_API_KEY` 或
...
理由是两家的共同点：pi 压根不把 key 存进自己的配置文件；CC 存了但**在源码注释里明说
那个文件不可信**。而 pai 阶段 4 要引入的 `~/.pai/settings.json` 是放**权限规则**的——
