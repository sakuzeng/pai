...
把这个问题推给了集成方（`AgentHarness` 两条队列的模式都可配，默认都是 `one-at-a-time`）。
CC 给了一个不同且更完整的答案，而且它是有真实用户量验证过的默认值。
...
与 pi 的根本差别：pi 用两个对象表达两种时机（`steeringQueue` / `followUpQueue`）；
CC 用一个对象 + 一个枚举字段。
...
`dequeue()` 是函数的语义，不是注入路径的行为。两个 drain 点实际都是批量：
...
即「批量 + 每条各自一条消息」是常规路径，逐条是为副作用隔离开的例外。
...
反过来，需要显式理由才降级到 `later` 的是系统消息，注释写明了动机：
...
全仓检索 `'now'`，能设置它的只有 SDK schema（`entrypoints/sdk/coreSchemas.ts`
的 `priority: z.enum(['now','next','later']).optional()`）。
REPL 与 print 只是订阅队列、看见 `now` 就 abort：
...
| `now` 档（abort 在跑的工具） | 不做 | pai 的中断是进程级标志（D#40），粒度对不上「只 abort 这一个工具调用」；而且 CC 自己也没把它开给交互式用户 |
...
pai 把 pi 的两层 while 压成了单层 `for` + `continue`（`loop.py:284-288`），
