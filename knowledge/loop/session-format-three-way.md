...
1. 首行 header。pi：`{type:"session", version, id, timestamp, cwd, parentSession?}`；
   dsh：header 带 `version`，JSONL 后端先于任何事件行解码就从原始 header 拒绝
...
4. 压缩是一条条目不是改写。pi `CompactionEntry`：`summary + firstKeptEntryId
...
- dsh：词汇表外的事件类型拒绝而非静默跳过（除非信封带 `ignorable: true`）——
