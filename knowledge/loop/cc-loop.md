...
**本篇与 `loop/cc-message-queue.md` 的分工**：那篇讲**队列**（一条队列、三档优先级、
默认值取舍），本篇讲**循环**（query 的边界、turn 怎么数、两条 drain 路径挂在循环的哪里）。
...
它们也读同一张 `PRIORITY_ORDER`，但**语义相反**：

| 用法 | 函数 | 语义 | 谁用 |
|---|---|---|---|
| **门槛切片** | `getCommandsByMaxPriority(max)` | 取**所有 ≤ max** 的，**不删** | Ⓐ（loop 内） |
| **取最高档** | `peek` / `dequeue` / `dequeueAllMatching` | 取**优先级最高**的那个/那批，**删** | 路 B（loop 外） |
...
| `getCommandsByMaxPriority('next')` | `[]` | 空数组上 `filter` 返回**新的空数组** |
...
`getQueuedCommandAttachments` 开头那句 `if (!queuedCommands) return []` 防的是**没传参**，
不是空数组——传 `[]` 时它走正常路径，只是 map 了零次。

**② `toolResults` 仍可能变长。** `getAttachmentMessages` 是**整条 attachment 流水线**，
...
**空集合走正常路径，比到处加空判断更不容易漏。** 唯一必须显式判空的是**有副作用的那一步**
...
如果模型这一轮**直接吐最终答案**，路 A 那段代码根本走不到。消息一直躺着，直到：
...
`queueProcessor.ts:36-44` 说明了**为什么批量是常规、逐条是例外**：
...
即：出队会改 snapshot → 触发 re-render → effect 又跑一次；但 `queryGuard.reserve()` 是**同步**
...
这是本篇最该记住的一句，也是与 pi 的**结构性**分歧（不是参数差异）：
...
`priority` 是 `QueuedCommand` 上的**可选**字段（`textInputTypes.ts:302-303`）：
...
> 它是**函数选择**的副产品：调 `enqueue` 就是 `next`，调 `enqueuePendingNotification` 就是 `later`。
...
**它没有修改任何一条命令的 `priority`**，改的是**读取时的门槛**：
...
后台任务完成走的是**另一个函数**（`messageQueueManager.ts:139-143`）：
...
于是即使你的 `'next'` 比它**晚**入队，`dequeue()` 按 `PRIORITY_ORDER`（`now:0 / next:1 / later:2`）
也**先给你**。——**人说话默认优先，机器说话默认等着。**

**例 4 · 可中断工具 —— 交互式用户唯一能碰到的「打断」**

`'now'` 你设不了（全仓只有 SDK schema 能设）。但提交路径上有一条**相邻的路**
...
在 CC 内部消息联合体里**平级**（`message.type === 'attachment'`）。
...
**最终形态还是 user 消息**——Anthropic API 不认识 attachment，它纯粹是**中间层**。
