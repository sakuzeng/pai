...
把「先讨论再动手」这类提示词约束降级到确定性层的机制载体就是 PreToolUse：
...
拒绝理由（reason 文本）作为工具结果回到模型上下文，引导它走合法路径——
...
任务路径、`!` 前缀显式放行留痕。注意分层：这是开发 pai 的工作区的门禁
...
## fail-open 还是 fail-closed：按失败的代价分场景（2026-08-11 追加）
...
后者选 fail-open 有个现实理由：一个写错的自律门禁会让开发整个停摆，
