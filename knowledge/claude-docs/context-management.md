...
MEMORY.md（只载前 200 行或 25KB，先到为准）~680、环境信息 ~280、MCP 工具**默认延迟
加载**只列名字 ~120、skills 只载一行描述 ~450、CLAUDE.md 全文。启动自动加载约占窗口
的一小部分，大头永远是对话消息与工具结果。

**compact 行为**：

- `/compact` 用一份**结构化摘要**替换整段对话。摘要保留：用户请求与意图、关键技术
  概念、检查/修改过的文件及重要代码片段、错误及修法、待办、当前工作。丢弃：完整
  工具输出、中间推理。模拟器按摘要 ≈ 被压内容的 **12%** 演示。
- 压缩后启动期自动加载的内容会**重新注入**（系统提示、CLAUDE.md、环境等），
