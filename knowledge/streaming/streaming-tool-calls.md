...
3. 并行调用是串行分片的（index=0 全部发完才开始 index=1），但不要依赖这一点——
...
在 `data: [DONE]` 之前多传一个额外的块，该块 `usage` 有值而 `choices` 始终是空数组；
其余块的 `usage` 为 `null`。
...
这段代码在 DeepSeek 上那个分支永不触发，`usage` 恒为 `None`。而 usage 通常喂给
