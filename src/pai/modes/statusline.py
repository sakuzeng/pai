...
# 宽度原语的家在 tui/（feature 12 T1）：九个 tui 模块都要用它，
# 住在 modes 里会让 tui 反向依赖 modes。这里 re-export 是为了不动九处调用点
# 与十处测试 import——搬家是结构修正，不是接口变更。
from pai.tui.width import ELLIPSIS, _ESCAPES, _truncate, display_width  # noqa: F401
